...
object kyu8_AWolfInSheepClothing extends App {
  ...
  def warnTheSheepCheck(queue: Array[String]): String = {
    if (queue.last == "wolf") "Pls go away and stop eating my sheep"
      ...
      val sheepIndex: Int = queue.length - queue.indexOf("wolf") - 1
      s"Oi! Sheep number ${sheepIndex}! You are about to be eaten by a wolf!"
    }
  }
